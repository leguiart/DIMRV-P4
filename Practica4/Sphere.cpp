...
void Sphere::init() {
	...
	for (int i = 0; i <= stacks; ++i) {
		...
		for (int j = 0; j <= slices; ++j) {
			...
			std::cout << vertexLC[i * (slices + 1) + j].position.x << ", " << vertexLC[i * (slices + 1) + j].position.y << ", " << vertexLC[i * (slices + 1) + j].position.z << ", " << std::endl;
		}
	...
	for (int i = 0; i < slices * stacks + slices; ++i) {
		...
		//std::cout << vertexLC[i].position.x << ", " << vertexLC[i].position.y << ", " << vertexLC[i].position.z << ", " << std::endl;
		//std::cout << vertexLC[i + slices +1].position.x << ", " << vertexLC[i + slices +1].position.y << ", " << vertexLC[i + slices +1].position.z << ", " << std::endl;
		//std::cout << vertexLC[i + slices].position.x << ", " << vertexLC[i + slices].position.y << ", " << vertexLC[i + slices].position.z << ", " << std::endl;
		//std::cout << vertexLC[i + 1].position.x << ", " << vertexLC[i + slices + 1].position.y << ", " << vertexLC[i + slices + 1].position.z << ", " << std::endl;
	}
}

glm::vec3 Sphere::searchVertex(glm::vec3 pos) {
	float posx = (float)pos.x;
	float posy = (float)pos.y;
	float posz = (float)pos.z;
	if(posz*posz + posy*posy + posx*posx > ratio)
		for (int i = 0; i < vertexLC.size() - slices - 1; ++i) {
			if (posx >= vertexLC[i].position.x && posx <= vertexLC[i + 1].position.x /*&& posz >= vertexNormal[i + slices].position.z && posz <= vertexNormal[i].position.z*/)
				if (posz <= vertexLC[i + slices].position.z && posz >= vertexLC[i].position.z)
				{
					std::cout << vertexLC[i].position.x << ", " << vertexLC[i].position.z << std::endl;
					return vertexLC[i].position;
				}
		}
	else
		return glm::vec3(0.0f, 1.0, 0.0);
}
