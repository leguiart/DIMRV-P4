...
public:
	...
	InputManager(glm::vec3 position, glm::vec3 up, Camera_Mode mode) {
		lastMousePos = glm::ivec2(0.0);
		mouseButtomState = new bool[3];
		keyState = new bool[1024];
		for (int i = 0; i < 3; i++)
		...
		for (int i = 0; i < 1024; i++)
			keyState[i] = false;
		cameraFPS = new Camera(mode, position, up);
	}
	...
	/*void setCameraData(/*Sphere& sphere, glm::vec3 pos) {
		//this->pos = pos;
		//std::cout << sphere.searchVertex(pos) << std::endl;
		//glm::vec3 position;
		//position = sphere.getVertexNormals()[sphere.searchVertex(pos)].position;
		//position.y += 0.5;
		cameraFPS = new CameraFPS(/*sphere, pos);
	}*/
	...
	void setMode(Camera_Mode mode) {
		this->cameraFPS->setCameraMode(mode);
	}
